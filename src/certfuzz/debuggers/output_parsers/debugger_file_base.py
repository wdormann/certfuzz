...
regex = {
    ...
    'signal': re.compile(r'(?:Program|Thread\s+\d+\s+"[^"]+")\s+received\s+signal\s+([^,]+)'),
    'exit_code': re.compile(r'exited with code (\d+)'),
    ...
    'mapped_frame': re.compile(r'^\s*(0x[0-9A-Fa-f]+)\s+(0x[0-9A-Fa-f]+)\s+0x[0-9A-Fa-f]+\s+((?:0x0)|(?:0x[0-9A-Fa-f]+\s+[rwxps-]*x[rwxps-]*))\s+(/.+)\s*$'),
